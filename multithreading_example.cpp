...
// Función que será ejecutada por el tercer hilo
void tarea_hilo_3_factorial(int n) {
    std::cout << "Hilo 3 (Factorial de " << n << "): Iniciando cálculo..." << std::endl;
...
int main() {
    ...
    // Inicia inmediatamente la ejecución de 'tarea_hilo_1_sleep_for'
    std::thread hilo1(tarea_hilo_1_sleep_for);

    // 2. Creación del segundo hilo, pasándole un argumento (el ID).
    // Inicia inmediatamente la ejecución de 'tarea_hilo_2_sum(42)'
    int thread_id = 42;
    std::thread hilo2(tarea_hilo_2_sum, thread_id);
    ...
    // 4. El hilo principal espera a que el 'hilo1' termine.
    ...
    // 5. El hilo principal espera a que el 'hilo2' termine.
    std::cout << "Hilo Principal: Esperando a que el Hilo 2 termine (join)..." << std::endl;
    hilo2.join();

    // 6. El hilo principal espera a que el 'hilo3' termine.
