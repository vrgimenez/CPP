...
#define _MOVE_CTOR_AND_ASSGN_OPE_   0
#define _UNIQUE_PTR_                1
...
#include <algorithm> // For std::copy
    ...
    #if _MOVE_CTOR_AND_ASSGN_OPE_ || _UNIQUE_PTR_
#include <utility>   // For std::exchange, std::move
    #endif  //#if _MOVE_CTOR_AND_ASSGN_OPE_ || _UNIQUE_PTR_
    #if _UNIQUE_PTR_
#include <memory>  // For std::unique_ptr
    #endif  //#if _UNIQUE_PTR_

class ColeccionEnteros {
private:
        #if _UNIQUE_PTR_
    std::unique_ptr<int[]> datos; // A unique_ptr to a dynamic array of ints
        #else
...
public:
        #if _CTOR_AND_DTOR_ || _COPY_CONSTRUCTOR_ || _COPY_ASSIGNMENT_OPE_ || _MOVE_CTOR_AND_ASSGN_OPE_ || _UNIQUE_PTR_
    // --- Constructores ---

    // 1. Constructor por Defecto (Default Constructor)
    // Inicializa una colección vacía.
    ColeccionEnteros() : datos(nullptr), tamano(0) {
        std::cout << "DEBUG: ColeccionEnteros Default Constructor called." << std::endl;
    }
    ...
    ColeccionEnteros(int n) : tamano(n) {
        if (tamano > 0) {
                #if _UNIQUE_PTR_
            std::cout << "DEBUG: ColeccionEnteros Parametrized Constructor called." << std::endl;
            if (tamano > 0) {
                datos = std::make_unique<int[]>(tamano); // Uses std::make_unique to create the array
            }
                #else
    ...
    // No Copy Constructor or Copy Assignment Operator needed! unique_ptr is not copyable.

    // Move Constructor (automatically generated by the compiler if we don't define it)
    ColeccionEnteros(ColeccionEnteros&& other) = default;

    // Move Assignment Operator (automatically generated by the compiler)
    ColeccionEnteros& operator=(ColeccionEnteros&& other) = default;
            #else
    // --- Destructor ---
    ...
    ~ColeccionEnteros() {
        if (datos != nullptr) {
            ...
            #endif  //#if _UNIQUE_PTR_
        #endif  //#if _CTOR_AND_DTOR_ || _COPY_CONSTRUCTOR_ || _COPY_ASSIGNMENT_OPE_ || _MOVE_CTOR_AND_ASSGN_OPE_ || _UNIQUE_PTR_
    ...
    void mostrarElementos() const {
        ...
        if (datos != nullptr) { // Safety check to prevent errors with moved-from objects
            for (int i = 0; i < tamano; ++i) {
                std::cout << datos[i];
                if (i < tamano - 1) {
                    std::cout << ", ";
...
ColeccionEnteros crearColeccionTemporal(int size) {
    ...
    #if _UNIQUE_PTR_
ColeccionEnteros crearColeccion() {
    ColeccionEnteros temp(3);
    temp.establecer(0, 10);
    temp.establecer(1, 20);
    temp.establecer(2, 30);
    return temp;
}
    #endif  //#if _UNIQUE_PTR_
...
int main() {
    ...
    // A. Using a local unique_ptr
    std::unique_ptr<int> ptr = std::make_unique<int>(100); // Create an int in the heap
    std::cout << "Value of ptr: " << *ptr << std::endl;

    // B. Using our ColeccionEnteros class
    ColeccionEnteros c1 = crearColeccion(); // Move from temporary to c1
    c1.mostrarElementos();

    // C. Moving ownership
    ColeccionEnteros c2 = std::move(c1); // Explicitly move ownership from c1 to c2
    std::cout << "c2 after move: "; c2.mostrarElementos();
    std::cout << "c1 after move (state is empty): "; c1.mostrarElementos();

    // D. The compiler prevents copying
    // ColeccionEnteros c3 = c2; // This would result in a compile error!
    #endif  //#if _MOVE_CTOR_AND_ASSGN_OPE_
